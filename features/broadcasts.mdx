...
Broadcasts fan out in the background without blocking the HTTP response. When sending via the public API, the recipient list is capped at **1,000 recipients per request**. For campaigns larger than 1,000 contacts, split the recipient list into batches of up to 1,000 and call the endpoint once per batch.

The server applies a per-user rate limit on broadcast initiation (independent of the per-message send limit) to protect your Meta API quota.

## API access

Broadcasts can be launched and monitored programmatically using a scoped API key with the `broadcasts:send` scope.

```bash
# Launch a broadcast
curl -X POST https://your-wacrm-instance.com/api/v1/broadcasts \
...
curl https://your-wacrm-instance.com/api/v1/broadcasts/<id> \
  -H "Authorization: Bearer <api_key>"
```

See the [Broadcasts API reference](/api/broadcasts) for the full request schema, response shape, and pagination details for listing past broadcasts.
